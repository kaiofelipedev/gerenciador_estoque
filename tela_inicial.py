from tkinter import *
import tela_login_adm as tla
import tela_login_user as tlu
...
class TelaInicial:
    def __init__(self, window):
        ...
        self.bt_adm = Button(self.frm, text="Administrador", bd=6, relief=RAISED,
                         font=("Verdana", 12, "bold"), background="#F2522E",activebackground="#F27B35", activeforeground="white", command=self.login_adm)
        self.bt_adm.place(relx=.1, rely=.5, relwidth=.35, relheight=.25)

        # Botão usuário

        self.bt_user = Button(self.frm, text="Usuário", relief=RAISED, bd=6,
                     font=("Verdana", 12, "bold"), background="#F2522E", activebackground="#F27B35",activeforeground="white", command=self.login_usuario)
        self.bt_user.place(relx=.55, rely=.5, relwidth=.35, relheight=.25)

    def login_adm(self):
        self.perfil_adm = 'adm'
        self.frm.place_forget()
        tla.TelaLoginAdm(self.window)
        #print(self.perfil_adm)

    def login_usuario(self):
        self.perfil_usuario = 'usuario'
        self.frm.place_forget()
        tlu.TelaLoginUser(self.window)
